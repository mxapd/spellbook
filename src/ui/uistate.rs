...
use crate::clipboard::ExecutionResult;
use crate::models::FocusTarget;
use crate::ui::add_spell_form::AddSpellForm;
use crate::ui::add_spellbook_form::AddSpellbookForm;
use crate::ui::confirm::ConfirmDialogState;
...
/// Top-level TUI state — the Elm "Model".
pub struct UiState {
    // Mode/overlay system
    pub mode: Mode,
...
impl UiState {
    ...
    pub fn search_query(&self) -> String {
        match &self.mode {
            Mode::BrowseSpellbooks(state) | Mode::BrowseSpells(state) => match state {
                BrowseState::Searching { query, .. } | BrowseState::SearchPaused { query, .. } => query.clone(),
    ...
    pub fn search_query_mut(&mut self) -> Option<&mut String> {
        match &mut self.mode {
            Mode::BrowseSpellbooks(BrowseState::Searching { query, .. } | BrowseState::SearchPaused { query, .. })
            | Mode::BrowseSpells(BrowseState::Searching { query, .. } | BrowseState::SearchPaused { query, .. }) => Some(query),
            _ => None,
        }
    }

    /// Start searching - transition to Searching state, preserving context
    pub fn start_search(&mut self) {
        match &mut self.mode {
            Mode::BrowseSpellbooks(state) => {
                let mut rs = ratatui::widgets::ListState::default();
                rs.select(None);
            ...
            Mode::BrowseSpells(state) => {
                // Preserve spellbook_index if coming from Viewing
                let idx = match state {
                    BrowseState::Viewing { spellbook_index, .. } => Some(*spellbook_index),
                    _ => None,
                };
                let mut rs = ratatui::widgets::ListState::default();
                rs.select(None);
    ...
    /// Stop searching - transition from Searching back to Idle or Viewing
    pub fn stop_search(&mut self) {
        match &mut self.mode {
            ...
            Mode::BrowseSpells(state) => {
                let idx = match state {
                    BrowseState::Searching { spellbook_index, .. }
                    | BrowseState::SearchPaused { spellbook_index, .. } => *spellbook_index,
                    _ => None,
                };
    ...
    /// Transition from Searching to SearchPaused (keeps filter, disables typing)
    pub fn pause_search(&mut self) {
        let new_state = match &self.mode {
            Mode::BrowseSpellbooks(BrowseState::Searching {
                ...
                results_state,
                spellbook_index,
            }) => Some(BrowseState::SearchPaused {
                query: query.clone(),
                filtered_indices: filtered_indices.clone(),
                filtered_spellbook_indices: filtered_spellbook_indices.clone(),
                results_state: results_state.clone(),
        ...
        if let Some(s) = new_state {
            match &mut self.mode {
                Mode::BrowseSpellbooks(state) | Mode::BrowseSpells(state) => *state = s,
                _ => {}
            }
        }
    }

    /// Transition from SearchPaused back to Searching, optionally appending a char
    pub fn resume_search(&mut self, ch: Option<char>) {
        let preserved = match &self.mode {
            Mode::BrowseSpellbooks(BrowseState::SearchPaused {
                query,
                filtered_indices,
                filtered_spellbook_indices,
                results_state,
                ...
                if let Some(c) = ch { q.push(c); }
                Some(BrowseState::Searching {
                    query: q,
                    filtered_indices: filtered_indices.clone(),
                    filtered_spellbook_indices: filtered_spellbook_indices.clone(),
                    results_state: results_state.clone(),
            ...
            Mode::BrowseSpells(BrowseState::SearchPaused {
                query,
                filtered_indices,
                filtered_spellbook_indices,
                results_state,
    ...
    /// Check if in searching state (not SearchPaused)
    ...
    /// Check if search is active or paused (i.e. search results are visible)
    ...
    pub fn filtered_indices_mut(&mut self) -> &mut Vec<usize> {
        match &mut self.mode {
            Mode::BrowseSpellbooks(BrowseState::Searching { filtered_indices, .. } | BrowseState::SearchPaused { filtered_indices, .. })
            | Mode::BrowseSpells(BrowseState::Searching { filtered_indices, .. } | BrowseState::SearchPaused { filtered_indices, .. }) => filtered_indices,
    ...
    pub fn filtered_spellbook_indices_mut(&mut self) -> &mut Vec<usize> {
        match &mut self.mode {
            ...
            Mode::BrowseSpellbooks(BrowseState::Searching { filtered_spellbook_indices, .. } | BrowseState::SearchPaused { filtered_spellbook_indices, .. })
            | Mode::BrowseSpells(BrowseState::Searching { filtered_spellbook_indices, .. } | BrowseState::SearchPaused { filtered_spellbook_indices, .. }) => filtered_spellbook_indices,
    ...
    pub fn search_results_state(&mut self) -> &mut ListState {
        match &mut self.mode {
            Mode::BrowseSpellbooks(BrowseState::Searching { results_state, .. } | BrowseState::SearchPaused { results_state, .. })
            | Mode::BrowseSpells(BrowseState::Searching { results_state, .. } | BrowseState::SearchPaused { results_state, .. }) => results_state,
    ...
    /// Get selected spellbook index (from Viewing, Searching, or SearchPaused)
...
mod tests {
    ...
    #[test]
    fn show_feedback_custom_duration() {
        let mut ui = UiState::new(false);
        ui.show_feedback(
            "short",
            FeedbackLevel::Info,
    ...
    #[test]
    fn tick_feedback_clears_expired() {
        let mut ui = UiState::new(false);
        ui.show_feedback(
            "expired",
            FeedbackLevel::Info,
    ...
    #[test]
    fn tick_feedback_clears_expired_flash() {
        let mut ui = UiState::new(false);
        ui.flash(
